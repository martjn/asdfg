...
const renderPage = (arg_id) => {
    ...
    localStorage.setItem('active_chapter_est', arg_id);
...
for (let i = 0; i < chapters.length; i++) {
    ...
    chapterFile.onreadystatechange = function() {
        if(this.readyState == 4 && this.status == 200) {
            ...
            else if (/^[A-Z][A-Z][A-Z]\.|[1-999]\.|\[/.test(line)) {
    ...
    chapterFile.open("GET", `./content_ee/chapter${chapter}.txt`, true);
    chapterFile.send();
}
...
document.addEventListener('DOMContentLoaded', () => {
    let currently_active_chapter = localStorage.getItem('active_chapter_est');
