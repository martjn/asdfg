...
let table_speech_text_est = '';
let table_speech_text_de = '';
...
xhttp.onreadystatechange = function() {
    if(this.readyState == 4 && this.status == 200){
        ...
        let results = [];
        table_speech_text_est = '';
        table_speech_text_de = '';
        ...
        searchField.addEventListener('input', function() {
            ...
            timeout = setTimeout(function() {
                ...
                if (searchTerm === '') {
                    ...
                    entriesCount.textContent = `${results.length} Texte gefunden - "${searchTerm}"`;
                    ...
                    for (let i = startIndex; i < endIndex; i++) {
                        ...
                        // Text-to-speech
                        table_speech_text_est += `${result.value.charAt(0).toUpperCase() + result.value.slice(1)}. `;
                        table_speech_text_de += `${result.valueDe.charAt(0).toUpperCase() + result.valueDe.slice(1)}. `;
                        ...
                        let listRowData3 = document.createElement('td');

                        let listRowHeaderText = document.createTextNode(`${result.tyybi_nr}`);
                        let listRowData1Text = document.createTextNode(`${result.value}`);
                        let listRowData2Text = document.createTextNode(`${result.valueDe}`);
                        let listRowData3Text = document.createTextNode(`${result.authentic_texts}`);
                        ...
                        listRowData3.appendChild(listRowData3Text);

                        listRow.appendChild(listRowHeader);
                        listRow.appendChild(listRowData1);
                        listRow.appendChild(listRowData2);
                        listRow.appendChild(listRowData3);

                        resultsList.appendChild(listRow);
                    }

                    table_speech_text_est = table_speech_text_est.replaceAll('®','').replace(/\r/g, '').replace(/\s+/g, ' ').replace(/ \. /g, '. ');
                    table_speech_text_de = table_speech_text_de.replaceAll('®','').replace(/\r/g, '').replace(/\s+/g, ' ').replace(/ \. /g, '. ');
                    console.log(table_speech_text_de);
...
xhttp.open('GET', './txt/vanasonadDe.txt', true);
xhttp.send();